...
const emptyForm = { name: "", calories: "", protein: "", carbs: "", fat: "", mealType: "breakfast" as Meal };
...
function Dashboard() {
  ...
  const total = summary?.total ?? { calories: 0, protein: 0, carbs: 0, fat: 0, count: 0 };
      ...
      {tab === "today" && (
        <>
          ...
          <section className="surface mt-4 p-4">
            ...
            {showAdd && (
              <form onSubmit={addEntry} className="mt-3 grid grid-cols-2 gap-2.5 sm:grid-cols-6">
                ...
                {form.name && form.calories && (
                  <button
                    type="button"
                    onClick={async () => {
                      await api.saveFavorite({ name: form.name, calories: Number(form.calories), protein: form.protein ? Number(form.protein) : 0, carbs: form.carbs ? Number(form.carbs) : 0, fat: form.fat ? Number(form.fat) : 0, mealType: form.mealType });
                      const { favorites: favs } = await api.listFavorites();
                      setFavorites(favs);
                      toast("Saved to favorites");
                    }}
                    className="col-span-2 text-xs text-emerald-600 hover:underline dark:text-emerald-400 sm:col-span-6"
                  >
                    ★ Save as favorite
                  </button>
                )}
